...
from motives.k_theory import Det, Dual, SymPower, Wedge, chern_character
...
def test_nonnegative_integer_tensor_powers(explicit_bundles):
    ...
    expected_square = (
        E.rank**2,
        2 * E.rank * E.ch(1),
        E.ch(1)**2 + 2 * E.rank * E.ch(2),
        2 * E.rank * E.ch(3) + 2 * E.ch(1) * E.ch(2)
    )
...
def test_determinant_character_is_exponential_of_first_component(explicit_bundles):
    """Verify that determinant character is the exponential of the first component."""
    _, E, _, _, _, eus, _ = explicit_bundles
    u1, _, _ = eus

    assert_tuple_equal(chern_character(Det(E)), (1, u1, u1**2 / 2, u1**3 / 6))


def test_derived_bundle_component_selection_and_truncation(explicit_bundles):
    """Verify component selection and truncation for duals and determinants."""
    _, E, _, _, _, eus, _ = explicit_bundles
    u1, u2, u3 = eus

    assert chern_character(Dual(E), component=3) == -u3
    assert_tuple_equal(chern_character(Dual(E), max_chern_degree=2), (E.rank, -u1, u2))
    assert chern_character(Det(E), component=2) == u1**2 / 2
    assert_tuple_equal(chern_character(Det(E), max_chern_degree=2), (1, u1, u1**2 / 2))


def test_nested_derived_bundle_character(explicit_bundles):
    """Verify characters of nested dual and determinant bundles."""
    _, E, _, _, _, eus, _ = explicit_bundles
    u1, _, _ = eus
    expected = (1, -u1, u1**2 / 2, -u1**3 / 6)

    assert_tuple_equal(chern_character(Det(Dual(E))), expected)
    assert_tuple_equal(chern_character(Dual(Det(E))), expected)
    assert_tuple_equal(chern_character(Det(Det(E))), chern_character(Det(E)))


def test_mixed_schemes_are_rejected_inside_derived_bundles(bundle_factory):
    """Verify that scheme validation unwraps dual and determinant bundles."""
    E = bundle_factory()
    F = bundle_factory()

    with pytest.raises(ValueError, match="same scheme"):
        chern_character(Dual(E) + Det(F))
