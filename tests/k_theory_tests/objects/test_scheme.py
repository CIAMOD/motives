...
from motives.k_theory import DeterminantBundle, DualBundle, Scheme, VectorBundle, chern_character, chern_class
...
def test_standard_derived_bundles_track_tangent_data_through_chern_engines():
    """Verify propagation from Tx through Tx_dual and Kx in every Chern interface."""
    X = Scheme("X_dynamic_geometry", 3)
    c1, c2, c3 = sp.symbols("c1 c2 c3")
    h1, h2, h3 = sp.symbols("h1 h2 h3")

    X.Tx.chern_classes = (1, c1, c2, c3)
    X.Tx.chern_character = (3, h1, h2, h3)

    assert X.Tx_dual.c() == (1, -c1, c2, -c3)
    assert X.Tx_dual.ch() == (3, -h1, h2, -h3)
    assert X.Kx.c() == (1, -c1, 0, 0)
    assert X.Kx.ch() == (1, -h1, h1**2 / 2, -h1**3 / 6)
    assert chern_class(X.Tx_dual) == (1, -c1, c2, -c3)
    assert chern_character(X.Tx_dual) == (3, -h1, h2, -h3)
    assert chern_class(X.Kx) == (1, -c1, 0, 0)
    assert chern_character(X.Kx) == (1, -h1, h1**2 / 2, -h1**3 / 6)
